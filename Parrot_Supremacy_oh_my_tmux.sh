if (env | grep TMUX); then
	...
	tmux new -d -s PARROT
	sleep 2
	tmux send-keys -t PARROT.1 'bash -c "$(curl -fsSL https://raw.githubusercontent.com/AfonsoMota-132/Parrot-supremacy/master/Parrot_Supremacy_oh_my_tmux.sh)"' ENTER
	tmux a -t PARROT
fi
